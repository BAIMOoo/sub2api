...
func TestBuildOpenAIImagesResponsesRequest_GenerationPreservesTransparentBackground(t *testing.T) {
	parsed := &OpenAIImagesRequest{
		Endpoint:     openAIImagesGenerationsEndpoint,
		Model:        "gpt-image-1.5",
		Prompt:       "transparent probe",
	...
	require.Equal(t, "medium", gjson.GetBytes(body, "tools.0.quality").String())
	require.Equal(t, "auto", gjson.GetBytes(body, "tools.0.size").String())
	require.Equal(t, "transparent probe", gjson.GetBytes(body, "input.0.content.0.text").String())
}
		...
		resp: &http.Response{
			...
			Body: io.NopCloser(strings.NewReader(
				"data: {\"type\":\"response.image_generation_call.partial_image\",\"partial_image_b64\":\"cGFydGlhbA==\",\"partial_image_index\":0,\"output_format\":\"png\"}\n\n" +
					"data: {\"type\":\"response.completed\",\"response\":{\"created_at\":1710000009,\"usage\":{\"input_tokens\":5,\"output_tokens\":9,\"output_tokens_details\":{\"image_tokens\":4}},\"tool_usage\":{\"image_gen\":{\"images\":1}},\"output\":[{\"type\":\"image_generation_call\",\"result\":\"ZmluYWw=\",\"output_format\":\"png\",\"size\":\"3840x2160\"}]}}\n\n" +
					"data: [DONE]\n\n",
			)),
		},
	}
